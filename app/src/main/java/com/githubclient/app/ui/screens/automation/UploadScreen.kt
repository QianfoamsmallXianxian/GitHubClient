...
fun UploadScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            // 统一间距：不再用 Spacer 手摆，字段之间固定 12dp
            ...
            OutlinedTextField(
                value = commitMessage, onValueChange = { commitMessage = it },
                label = { Text("提交信息") }, singleLine = true,
                modifier = Modifier.fillMaxWidth()
            )
            OutlinedTextField(
                value = content,
            ...
            Button(
                ...
                enabled = owner.isNotBlank() && repo.isNotBlank() && path.isNotBlank() &&
                        content.isNotBlank() && state !is UploadState.Loading,
                modifier = Modifier.fillMaxWidth()
            ) {
                if (state is UploadState.Loading) {
                    // 进度圈用 size 而不是 height，否则会被拉成椭圆
